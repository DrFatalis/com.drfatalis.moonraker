...
class MoonrakerClient extends EventEmitter {
  ...
  // ─── HTTP polling helpers (no WebSocket involved) ────────────────────────────

  async httpListObjects() {
    const data = await this.httpGet('/printer/objects/list');
    const result = data.result || data;
    return result.objects || [];
  }

  async httpQueryObjects(objects) {
    const query = Object.entries(objects)
      .map(([name, attrs]) => (Array.isArray(attrs) && attrs.length)
        ? `${encodeURIComponent(name)}=${attrs.map(encodeURIComponent).join(',')}`
        : encodeURIComponent(name))
      .join('&');
    const data = await this.httpGet(`/printer/objects/query?${query}`);
    return data.result || data;
  }
  ...
  _openSocket() {
    ...
    // Forcibly close (not just detach) any lingering socket before creating a new
    // one. A previous attempt may still be mid-handshake (e.g. Moonraker is
    // restarting and the TCP connection never resolved) — merely detaching its
    // listeners would abandon that socket without closing it, leaking a
    // connection on every retry. Repeated reconnect attempts during a Moonraker
    // update/restart would then pile up half-open sockets and can exhaust the
    // connection limit on Moonraker's reverse proxy (nginx in Fluidd/Mainsail),
    // preventing the real reconnect from ever completing its handshake.
    this._closeSocket();
  ...
  _closeSocket() {
    if (this._ws) {
      this._detachSocket();
      try {
        // ws-module sockets support terminate() for an immediate hard close that
        // doesn't wait on a graceful handshake (important for sockets that are
        // still CONNECTING). Native WebSocket only has close().
  ...
  _startPing() {
    ...
    this._pingTimer = setInterval(() => {
      ...
      this.request('server.info').catch(() => {
        // Ping timed out — the socket is silently dead (e.g. host power-cycled
        // without a clean TCP close).  Force a disconnect so the normal
        // reconnect loop takes over.
        if (!this._destroyed && this._connected) {
          this._log.log('[MoonrakerClient] Ping failed, forcing reconnect');
          this._connected = false;
          this._stopPing();
          this._closeSocket();
          this._rejectAll(new Error('Ping failed'));
          this._scheduleReconnect();
